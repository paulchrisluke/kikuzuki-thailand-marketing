...
export const requests = sqliteTable("requests", {
 ...
 check("requests_kind_check", sql`kind IN ('contact', 'reservation', 'experience_booking', 'work')`),
...
export const mcp_tool_call_events = sqliteTable("mcp_tool_call_events", {
	...
	check("mcp_tool_call_events_surface_check", sql`mcp_surface IN ('client', 'public_help')`),
...
export const content_documents = sqliteTable("content_documents", {
	...
	check("content_documents_kind_check", sql`kind IN (${sql.raw(CONTENT_DOCUMENT_KINDS.map(kind => `'${kind}'`).join(', '))})`),
...
export const content_blocks = sqliteTable("content_blocks", {
	...
	check("content_blocks_type_check", sql`type IN (${sql.raw(CONTENT_BLOCK_TYPES.map(type => `'${type}'`).join(', '))})`),
