...
export const PRODUCTS_TOOLS: McpToolDefinition[] = [
  ...
  siteTool({ name: 'reconcile_products', description: 'Create and update the supplied products at one explicit location in a single atomic reconciliation. This is a one-time write, not ongoing synchronization. Omitted products remain available unless set_missing_unavailable is explicitly true. Read every list_location_products page first. Any invalid row rolls back the complete request. Every row must include price; use null when the intended state has no active fixed Price.', domain: 'products', minimumRole: 'editor', confirmRequired: false, inputSchema: { location_id: { type: 'string' }, products: { type: 'array', maxItems: PRODUCT_LIMITS.reconcile, items: { type: 'object', properties: { product_id: { type: 'string', minLength: 1 }, ...productWrite }, required: ['category_id', 'name', 'price'], additionalProperties: false } }, set_missing_unavailable: { type: 'boolean', description: `When true, mark stored Products omitted from this complete request unavailable. At most ${PRODUCT_LIMITS.reconcile} intended rows may be supplied.` } }, required: ['location_id', 'products'], outputSchema: { type: 'object', properties: { products: { type: 'array', items: productObject } }, required: ['products'] } }),
]
