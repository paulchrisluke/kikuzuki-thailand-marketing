...
export const PRODUCTS_TOOLS: McpToolDefinition[] = [
  ...
  siteTool({ name: 'import_products_from_media', description: 'Send one stored image or PDF to Anthropic through Cloudflare AI Gateway, charge AI credits, and atomically create validated Products at one explicit location. Empty or partially invalid extraction is an error and can still consume credits.', domain: 'products', minimumRole: 'editor', confirmRequired: true, inputSchema: { location_id: { type: 'string' }, asset_id: { type: 'string' } }, required: ['location_id', 'asset_id'], outputSchema: { type: 'object', properties: { products: { type: 'array', items: productObject }, creditsRemaining: { type: 'number' } }, required: ['products', 'creditsRemaining'] } }),
]
