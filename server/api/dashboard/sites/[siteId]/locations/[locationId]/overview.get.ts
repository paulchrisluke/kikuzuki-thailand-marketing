...
export default defineHandler(async (event) => {
  ...
  if (!siteId || !locationId) throw new HTTPError({ statusCode: 400, statusMessage: 'Site ID and Location ID are required' })
  const includeProducts = getQuery(event).includeProducts !== 'false'
  return jsonResponse(await loadDashboardLocationOverview(event, siteId, locationId, { includeProducts }))
})
