...
export default defineHandler(async (event) => {
  ...
  const billing = await getOrganizationBillingStatus(env, db, orgId)
  ...
  return jsonResponse({
    org_name: organization.name, org_slug: organization.slug, stripe_customer_id: billing.stripeCustomerId, stripe_subscription_id: billing.stripeSubscriptionId, plan: billing.plan, status: billing.subscriptionStatus, current_period_end: billing.currentPeriodEnd, cancel_at_period_end: billing.cancelAtPeriodEnd, sites_billing: (sites ?? []).map(site => ({
      ...site, stripe_subscription_id: billing.stripeSubscriptionId, plan: billing.plan, status: billing.subscriptionStatus, current_period_end: billing.currentPeriodEnd, cancel_at_period_end: billing.cancelAtPeriodEnd, })), pending_transfer: transfer
      ? {
