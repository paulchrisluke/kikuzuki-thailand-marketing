...
import { listExperienceBookings, updateBookingStatus } from '~/server/utils/experiences'
...
export default defineEventHandler(async (event) => {
  ...
  // PATCH support on same route for updating booking status
  if (event.method === 'PATCH') {
    ...
    const ok = await updateBookingStatus(db, siteId, body.booking_id, body.status as 'pending' | 'confirmed' | 'cancelled')
