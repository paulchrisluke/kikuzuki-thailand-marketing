...
import { publishTranslationDrafts, type TranslationScope } from '~/server/utils/translation-inventory'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    const result = await publishTranslationDrafts(db, site.organization_id, siteId, body.locale, parseScope(body.scope), session.user.id)
    await upsertSiteLocale(db, site.organization_id, siteId, {
