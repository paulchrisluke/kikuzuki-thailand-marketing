...
The retained public-CIMD OAuth test now performs bearer-authenticated MCP initialization before listing tools. It reuses the exchanged token and clears cookies explicitly. The complete OAuth file passed locally with eight passes and one HTTPS-only skip, closing a coverage gap without requiring an operator to obtain CI credentials. Its deployed result remains part of normal CI.
...
The exact c7 preview passed all 544 expanded public-route cases with zero retries or skips on Nuxt build `cc7cfc87-4bc0-48e3-883a-c24f2b714411`. The later application commit f5c90696 passed all 22 affected public-route cases and manual booking/scheduling checks on preview build `3de85670-3a3e-47d5-8b2b-a30f2c8f889f`, after convergence across 633 assets. NCLS video playback was also verified in Chrome. Separate headless player failures remain recorded with their cause unproven. The final test-only addition still requires its normal CI result.
