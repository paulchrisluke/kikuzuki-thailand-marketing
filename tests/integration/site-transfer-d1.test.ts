...
test('site transfer atomically moves owned aggregates and revokes access while retaining historical facts', { timeout: 90_000 }, async () => {
  ...
  try {
    ...
    await db.prepare("INSERT INTO review_requests (id,organization_id,site_id,location_id,customer_id,booking_type,booking_id,token_hash,expires_at) VALUES ('review-request','sender','site','location','customer','experience_booking','booking','review-token','2099-01-01T00:00:00.000Z')").run()
