...
| Inline organization expansion and client actions | Inspect an organization and operate its workspace | Detail state was coupled to the old collection pages | Sub-hub | `/admin/organizations/:id` | `EditorNavigationList`, `EditorPaneShell` | Move to one refresh-safe organization sub-hub |
| Inline organization overview | Inspect sites, locations, and client state | Nested in the collection | Durable leaf | `/admin/organizations/:id/overview` | `EditorPaneShell` | Move under the canonical organization record |
...
| `/admin/members` | Review and add platform staff | Bespoke card and inline form | Browse + transient task | `/admin/members`, invite dialog | `EditorNavigationList`, `DashboardListItemDialog` | Replace |
