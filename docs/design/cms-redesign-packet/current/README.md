...
## Previously blocked — both resolved

| Route | Outcome |
|---|---|
| `.../locations/[locationSlug]/products` **selected-item edit state** (both locations) | **Not applicable.** This waited on the per-item edit form behind issue [#723](https://github.com/paulchrisluke/krabiclaw/issues/723). #723 was fixed in `9f4a688f` (2026-09-02) and closed 2026-09-04, but by then the CMS redesign had removed the form itself: a dish is now its own hub of single-concern leaves at `products/[categoryId]/[productId]/<section>`, so there is no slideover left to capture. |
| `.../locations/[locationSlug]/settings/features` **the actual working Available Features screen** | **Captured** at `sites/[siteSlug]/locations/[locationSlug]/settings/features/index-working.jpg`. Issue [#720](https://github.com/paulchrisluke/krabiclaw/issues/720) was fixed and closed, but the screen still 500'd on a second, unrelated cause: `LocationSettingsPage.vue` carried its own copy of the module-label table, listing `menu` — a label — where the feature id is `products`, so the id it actually receives had no entry and the component threw. Labels now come from `resolveCmsCapabilities`, which is where a module's per-vertical name already lives. This is the packet's one **local dev** capture; the fix ships alongside it and was not in production when the screenshot was taken. |
...
- **Two real production bugs** were found and filed while capturing, unrelated to the redesign itself: Location Settings → Available features threw a 500 on both locations ([#720](https://github.com/paulchrisluke/krabiclaw/issues/720)); the Menu editor's product list broke on both locations with a raw error string surfaced to the user ([#723](https://github.com/paulchrisluke/krabiclaw/issues/723)). Both were fixed in `9f4a688f` on 2026-09-02 and closed 2026-09-04. The Available Features screen then turned out to have a *second* 500 behind the first — see "Previously blocked" above — which is fixed in the same change that captures it.
