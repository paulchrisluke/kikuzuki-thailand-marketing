...
| `.../locations/[locationSlug]/settings/features` **the actual working Available Features screen** | **Captured** at `sites/[siteSlug]/locations/[locationSlug]/settings/features/index-working.jpg`. Commit `9f4a688f` fixed issue [#720](https://github.com/paulchrisluke/krabiclaw/issues/720), but the same defect was later reintroduced: `LocationSettingsPage.vue` carried its own copy of the module-label table, listing `menu` — a label — where the feature id is `products`, so the id it actually receives had no entry and the component threw. Labels now come from `resolveCmsCapabilities`, which is where a module's per-vertical name already lives. This is the packet's one **local dev** capture; the fix ships alongside it and was not in production when the screenshot was taken. |
...
- **Two real production bugs** were found and filed while capturing, unrelated to the redesign itself: Location Settings → Available features threw a 500 on both locations ([#720](https://github.com/paulchrisluke/krabiclaw/issues/720)); the Menu editor's product list broke on both locations with a raw error string surfaced to the user ([#723](https://github.com/paulchrisluke/krabiclaw/issues/723)). Both were fixed in `9f4a688f` on 2026-09-02 and closed 2026-09-04. The Available Features label-table defect was later reintroduced. The registry-based fix and working capture are described under "Previously blocked" above.
