...
- **The Available Features working screen** is captured at `current/sites/[siteSlug]/locations/[locationSlug]/settings/features/index-working.jpg`. It is the one screenshot in `current/` taken on **local dev rather than production**, and deliberately so: the screen was still throwing a 500 in production, from a second cause unrelated to [#720](https://github.com/paulchrisluke/krabiclaw/issues/720), and the fix for it ships in the same change as this capture. Every other row in `current/` remains a production capture.
