        ...
        <template #detail>
          ...
          <div v-else-if="editorKey === 'publishing'" class="space-y-6">
            ...
            <template v-else>
              ...
              <UFormField v-if="editor.form.topic.scheduled_for" label="Goes live" required description="Must be in the future.">
                <UInput
                  :model-value="toLocalDateTimeInput(editor.form.topic.scheduled_for)"
                  type="datetime-local"
                  class="w-full"
                  @update:model-value="editor.form.topic.scheduled_for = fromLocalDateTimeInput(String($event))"
                />
              </UFormField>
            </template>
...
function publishingSummary(): string {
  ...
  return Number.isNaN(when.getTime())
    ? 'Not scheduled'
    : `Goes live ${when.toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' })}`
}
