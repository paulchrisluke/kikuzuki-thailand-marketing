  ...
  <div class="space-y-5">
    ...
    <div class="grid gap-4 sm:grid-cols-2">
      ...
      <UFormField label="Start time" required>
        <UInput
          :model-value="event.schedule.start_time"
          type="time"
          step="any"
          class="w-full"
      ...
      <UFormField label="End time" required>
        <UInput
          :model-value="event.schedule.end_time"
          type="time"
          step="any"
          class="w-full"
    ...
    <UFormField
      ...
      label="Stops repeating on (UTC)"
      help="Optional. Leave empty to repeat until you remove it."
    >
      <UInput
        :model-value="event.recurrence_info.series_end_time ? instantDate(event.recurrence_info.series_end_time).toISOString().slice(0, -1) : ''"
        type="datetime-local"
        step="any"
        class="w-full"
...
import { fromTimeInput } from '~/utils/post-fields'
...
function setSeriesEnd(value: string) {
  const rule = event.value.recurrence_info
  if (!rule) return
  const iso = value ? scheduledLifecycleValue('Scheduled', value, 'UTC') : null
  if (iso) {
