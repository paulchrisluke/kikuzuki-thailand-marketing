...
for (const col of columns) {
  try {
    console.log(`Checking ${table} for ${col.name}...`);
    const info = execSync(`npx wrangler d1 execute REVIEWS_DB --local --command "PRAGMA table_info(${table});" --json`).toString();
    ...
    if (!existingColumns.some(c => c.name === col.name)) {
      ...
      execSync(`npx wrangler d1 execute REVIEWS_DB --local --command "ALTER TABLE ${table} ADD COLUMN ${col.name} ${col.type};"`);
      console.log(`Done.`);
    } else {
