  ...
  test_cases: [
    {
      ...
      user_prompt: 'Show me my business sites, ask which site I want to work on, and list its locations after I choose.',
      file_attachment_urls: null,
      tools_triggered: 'get_workspace_context, list_sites, set_workspace_context, list_locations',
      expected_output: 'Returns only the authenticated owner\'s sites and the locations for the selected site.',
      expected_output_url: null,
    },
    {
      description: 'Using a disposable reviewer site and location explicitly selected through the app, create a product with a customer-facing price note.',
      user_prompt: 'At the site and location I selected, create a category named Submission Sushi and add a product named Chef\'s Choice with no numeric price and the exact wording “Market Price.”',
      file_attachment_urls: null,
      tools_triggered: 'create_product_category, create_product',
      expected_output: 'Creates the product in the explicitly selected location and new category with no fixed price and preserves “Market Price” as the customer-facing price wording.',
      expected_output_url: null,
    },
    {
      description: 'On an explicitly selected disposable reviewer location, reconcile the complete catalog after reading every existing page.',
      user_prompt: 'At the site and location I selected, create a category named Submission Drinks. Make the complete intended product list: Espresso at 80 THB, Tea at 60 THB, and Water at 20 THB, all per item and tax-inclusive in that category. Show me the proposed reconciliation and which existing products will become unavailable. Apply it only after I confirm.',
      file_attachment_urls: null,
      tools_triggered: 'list_location_products, list_product_categories, create_product_category, sync_products',
      expected_output: 'After explicit approval, atomically reconciles the selected location using the new category, marks omitted products unavailable, and reports all three prices accurately; no write happens while confirmation is pending.',
      expected_output_url: null,
    },
    {
      description: 'Preview announcement copy in chat, then publish it to the explicitly selected disposable reviewer site after approval.',
      user_prompt: 'For the site I selected, propose a post titled Submission test with the body “This is a demonstration announcement.” Show the copy here without saving it, then publish it to the website only after I confirm.',
      file_attachment_urls: null,
      tools_triggered: 'create_post',
      expected_output: 'Shows proposed copy in chat with no mutation; after approval, create_post publishes immediately and returns the created post ID and public URL.',
      expected_output_url: null,
    },
    {
      description: 'Create a bookable experience at an explicitly selected disposable reviewer site and location.',
      user_prompt: 'At the site and location I selected, create an experience titled Submission cooking class lasting two hours, priced at 1,500 THB per person excluding tax.',
      file_attachment_urls: null,
      tools_triggered: 'create_experience',
      expected_output: 'Creates the experience only at the explicitly selected location with the supplied duration and fixed Price semantics.',
      expected_output_url: null,
    },
  ],
  negative_test_cases: [
