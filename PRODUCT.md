...
The primary users are owners and operators of local and professional-service
businesses. They create and maintain a public website through ChatGPT or the
dashboard CMS without needing to operate a conventional website builder.

KrabiClaw platform administrators are an internal audience. They manage
organizations, people, support work, domains, platform publishing, and product
operations through the shared dashboard shell and the platform-admin MCP app.
...
Success means a business owner can publish and operate an accurate,
tenant-specific site through either supported editing interface while every
interface reads and writes the same canonical tenant state.

## Positioning

KrabiClaw is ChatGPT-first rather than form-first. Its Client MCP app and
dashboard CMS are two interfaces over the same server and domain behavior, not
separate products or content models. The product combines conversational site
creation with an authenticated CMS, organization-level billing, and
vertical-specific public templates.

## Operating Context

Business owners work in ChatGPT for conversational setup and content changes,
or in the dashboard for structured editing, billing, inbox, analytics, and
organization settings. Platform administrators use the same dashboard shell
for cross-tenant operations and a separate restricted MCP app for platform
publishing. Public sites render tenant content through the Saya or Blawby
template selected by the canonical template registry.

## Capabilities and Constraints

- KrabiClaw supports the `restaurant`, `experience`, and `service` verticals.
- One Better Auth organization owns membership, billing, permissions, and one
  or more sites. Sites and locations remain explicit routing contexts.
- ChatGPT MCP, dashboard CMS, ChowBot, and WhatsApp use shared server and domain
  utilities with one canonical state.
- Public pages render validated tenant content. Missing tenant content must not
  be replaced with fabricated examples or fallback copy.
- Better Auth owns identity, sessions, organizations, teams, roles,
  permissions, impersonation, and OAuth resource-server behavior.
- Cloudflare Workers, D1, R2, Images, and AI Gateway are the deployed runtime.
- The detailed vertical, billing, routing, integration, and architecture
  contracts below remain authoritative within this product record.

## Brand Commitments

KrabiClaw is the product name. Saya is the public template for restaurant and
experience tenants; Blawby is the public template for professional-service
tenants. Tenant sites must use the client's real content and media. Product UI
uses the tenant's vocabulary rather than exposing storage terminology.

## Evidence on Hand

- `CONTEXT.md` records the canonical domain and vertical contracts.
- `DESIGN.md` records the dashboard CMS navigation and editing contract.
- `chatgpt-app-submission.json` records the Client MCP review surface.
- Pottery House Krabi, Kikuzuki, and NCLS provide real tenant content and
  production-like verification targets. Their presence is not permission to
  invent claims, testimonials, or missing client content.

## Product Principles

- Keep one canonical state across every editing and messaging interface.
- Make organization, site, and location scope explicit rather than inferred.
- Let business owners work in their own vocabulary and choose between
  conversational and structured editing.
- Publish only validated tenant content and preserve tenant isolation.
- Extend the product through shared domain behavior and registry-driven
  variation instead of parallel implementations.
